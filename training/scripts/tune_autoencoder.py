"""
Hyperparameter tuning cho Autoencoder
...
def train_model(train_embeddings: np.ndarray,
...
def grid_search_autoencoder(train_embeddings: np.ndarray,
                            val_embeddings: np.ndarray,
    ...
    """
    Grid search cho Autoencoder hyperparameters
    """
    logger.info("\n" + "="*70)
    logger.info("GRID SEARCH - AUTOENCODER HYPERPARAMETER TUNING")
    logger.info("="*70)
    ...
    save_dir = Path(save_dir) / "autoencoder"
...
def visualize_tuning_results(results: List[Dict], save_path: Path):
    ...
    # Plot 1: Latent dimension vs Val Loss
    axes[0, 0].scatter(latent_dims, val_losses, alpha=0.6, s=100)
    ...
    axes[0, 0].set_title('Latent Dimension vs Validation Loss', fontsize=14, fontweight='bold')
    axes[0, 0].grid(True, alpha=0.3)
    
    # Plot 2: Learning Rate vs Val Loss
    axes[0, 1].scatter(learning_rates, val_losses, alpha=0.6, s=100, color='orange')
    ...
    axes[0, 1].set_title('Learning Rate vs Validation Loss', fontsize=14, fontweight='bold')
...
def tune_autoencoder_local(dataset_name: str = "HDFS",
                          log_file: str = None,
                          epochs_per_config: int = 10,
                          device: str = "cpu",
                          save_dir: str = "output/tuning_results") -> Dict:
    """
    Tune Autoencoder hyperparameters trên local dataset
    """
    # Tìm log file
    if log_file is None:
    ...
    train_embeddings = embeddings[:-n_val]
    val_embeddings = embeddings[-n_val:]
    
    logger.info(f"Train samples: {len(train_embeddings)}, Validation samples: {len(val_embeddings)}")
    ...
    # Grid search
    results = grid_search_autoencoder(
        train_embeddings=train_embeddings,
        val_embeddings=val_embeddings,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Tune Autoencoder hyperparameters')
